...
void loop()
{
    ...
    {
    case WAITING_AGENT:
        // ping every 1s until agent available
    ...
    {
        // create entity, if success -> CONNECTED else back to WAITING_AGENT
        bool ok = createEntities();
        state = ok ? AGENT_CONNECTED : WAITING_AGENT;
    ...
    case AGENT_CONNECTED:
        // ping every 200ms, if fail → DISCONNECTED
        ...
        if (state == AGENT_CONNECTED)
        {
            ...
            current_time = millis();
            // ถ้าเกิน 100ms ไม่ได้รับคำสั่งใหม่ ให้หยุดรถ
            if ((current_time - prev_cmd_time) > 100)
            {
                fullStop();
            }
        }
...
bool createEntities()       // create ROS entities 
{
    ...
    RCCHECK(rclc_executor_init(&executor, &support.context, 7, &allocator));    // max handles = 7
...
void Move()
{
    ...
    // m/s to rpm
    float wheel1_rpm = (wheel_left / (0.2 * M_PI * WHEEL_RASDIUS)) * 60.0f; // front left
    float wheel2_rpm = (wheel_right / (0.2 * M_PI * WHEEL_RASDIUS)) * 60.0f; // front right
    float wheel3_rpm = (wheel_left / (0.2 * M_PI * WHEEL_RASDIUS)) * 60.0f; // rear left
    float wheel4_rpm = (wheel_right / (0.2 * M_PI * WHEEL_RASDIUS)) * 60.0f; // rear right
...
void publishData()
{
    ...
    debug_motor_msg.linear.y = motor_msg.linear.y;
...
{
    struct timespec ts;
    if (RCL_RET_OK != rcl_uros_epoch_time(&ts))
    {
        ts.tv_sec = 0;
        ts.tv_nsec = 0;
    }
...
void rclErrorLoop()
{
    Serial.println("Error in micro-ROS, looping forever...");
    while (1)
        ;
}
