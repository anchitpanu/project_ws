...
void loop() {
  switch (state) {
    ...
    case AGENT_CONNECTED:
      EXECUTE_EVERY_N_MS(500, state = (RMW_RET_OK == rmw_uros_ping_agent(1000, 10)) ? AGENT_CONNECTED : AGENT_DISCONNECTED;);
...
bool createEntities() {
  ...
  const unsigned int control_timeout = 150;
...
void publishData() {
  encoder_msg.linear.x = motor_msg.linear.x;  
